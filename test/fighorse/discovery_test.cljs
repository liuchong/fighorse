...
(deftest manifest-is-self-describing
  (testing "manifest exposes workflow and contracts"
    (let [m (discovery/manifest)]
      ...
      (is (= "fighorse.setup-guidance.v1" (get-in m [:setup_requirements :kind])))
      (is (clojure.string/includes? (get-in m [:setup_requirements :ai_client_behavior :if_auth_missing])
                                    "fighorse auth login"))
    ...
    (with-discovery-env
      (fn [_home]
        (let [url "https://www.figma.com/design/abc123/Test?node-id=1-2"
          ...
          (is (= "fighorse.setup-guidance.v1" (get-in report [:setup :kind])))
          (is (clojure.string/includes? (get-in report [:setup :ai_client_behavior :if_auth_missing])
                                        "FIGMA_TOKEN"))
    ...
    (with-discovery-env
      (fn [_home]
        (let [report (discovery/doctor)]
          ...
          (is (clojure.string/includes? (get-in report [:auth :missing_token_ai_prompt])
                                        "fighorse auth login"))
          (is (= "fighorse.setup-guidance.v1" (get-in report [:setup :kind])))
