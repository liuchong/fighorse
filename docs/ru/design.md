...
`doctor` дополняет discovery runtime-статусом: информация о Bun/runtime, статус auth, home directory, MCP mode, local-write mode и готовность experience-store.
