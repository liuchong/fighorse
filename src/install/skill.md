...
Figma file-browser links are not design targets. `parse_figma_url` can extract `team_id` from `/files/.../team/<team-id>`; enumerate visible resources with `get_team_projects`, then `get_project_files` for every project. This requires the token scope `projects:read` and may require Figma Projects endpoint approval; HTTP 403 can also mean the token user cannot access that team. A `/files/<browser-root>` URL without `/team/<team-id>` cannot discover teams through the public REST API, so ask for a team URL or a concrete file/selection URL. Never record real browser IDs, team IDs, project IDs, file keys, names, or URLs in reusable experience.
