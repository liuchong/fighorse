...
//!
//! Shared machine-readable guidance. These are static JSON payloads plus the
...
pub fn export_dirs() -> Value {
    json!({
        "recommended_roots": [
            "<project>/.fighorse/exports",
            "<project>/assets/fighorse",
            "~/.fighorse/exports"
        ],
        "default_cli_dir": "./.fighorse/exports",
        "rules": [
            "Prefer a project-local directory when implementing app UI so generated files are easy to reference from source code.",
...
/// The fighorse AI contract.
pub fn ai_contract() -> Value {
    json!({
        ...
        "must_not": [
            "Do not silently choose a platform, framework, density, or asset format.",
        ...
        "export_policy": export_dirs(),
        "fallback": "If any required detail is missing, ask a concise question instead of guessing."
    })
}

/// Output-location guidance including the resolved fighorse home.
pub fn output_location_guidance() -> Value {
    json!({
        "kind": "fighorse.output-location-guidance.v1",
        "fighorse_home": config::fighorse_home().to_string_lossy(),
        "export_policy": export_dirs(),
        "examples": [
            {"purpose": "temporary Figma slices and debug screenshots",
